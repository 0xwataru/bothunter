...
def setPaths():
    ...
    paths.P2P_PATH = os.path.join(root_path, "p2pfinder")
    paths.C2_PATH = os.path.join(root_path, "c2finder")
    paths.OUTPUT_PATH = os.path.join(root_path, "output")
    paths.CONFIG_PATH = os.path.join(root_path, "config.ini")
    ...
    paths.SALITY_PCAP = os.path.join(root_path, "sample\\pcap\\sality_sample.pcap")
    paths.GOZ_SAMPLE = os.path.join(root_path, "sample\\file\\goz_unpack.memdump")
    # paths.SALITY_PCAP = os.path.join(root_path, "sample/pcap/sality_sample.pcap")
    # paths.GOZ_SAMPLE = os.path.join(root_path, "sample/file/goz_unpack.memdump")
    # paths.UA_LIST_PATH = os.path.join(paths.DATA_PATH, "user-agents.txt")

    if os.path.isfile(paths.CONFIG_PATH) and os.path.isfile(paths.SALITY_PCAP) and os.path.isfile(
            paths.GOZ_SAMPLE):
        pass
    else:
